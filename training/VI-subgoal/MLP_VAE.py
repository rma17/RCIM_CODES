...
latent_dim = 3
epochs = 100
vae=VAE_Linear(num_feature, latent_dim, original_dim, 5)
model=VAE_Regression(vae,device)
optim = torch.optim.Adam(vae.parameters(), lr=0.001)
model=VAE_Regression(vae,device)
for train_index, test_index in kf.split(X):
    
    X_train,Y_train,Label_train=X[train_index],Y[train_index],label[train_index]
    model.fit(32,loss_func,optim,X_train,Y_train,2000,Label_train)
    X_test,Y_test,Label_test=X[test_index],Y[test_index],label[test_index]
    m,r,out= model.test(X_test,Y_test,Label_test )
   
    # # if r>=0.940:
    #     break
    
# # model.fit(16,loss_func,optim,X,Y,2500,label)
# # # # X_test,Y_test,Label_test=X[test_index],Y[test_index],label[test_index]
m,r,out=model.test(X,Y,label)


torch.save(vae.state_dict(), 'sub1.pt')





# out=vae(obs.float(),label.float())[1]
# out=out.cpu().detach().numpy()
# tsne = MDS(n_components=2, random_state=0)
# X_3d = tsne.fit_transform(out)
